...
- 2026-03-13: The session-management hardening slice is now in place as well. Session metadata now supports pin/archive state, transfers can round-trip through explicit export/import bundles, and the history panel can search across active plus archived sessions without pushing more state back into `TaskController`.
- 2026-03-13: The roadmap now has a concrete IDE-first bootstrap slice as well. Windows shell execution is routed through a single envelope contract, provider model fetch ownership is less self-triggered, and the repo now contains both a shared-core contract package plus a buildable Electron + Monaco desktop workbench scaffold.
