import * as vscode from 'vscode';
import * as https from 'https';
import * as fs from 'fs';
import * as path from 'path';
import * as os from 'os';
import { execFile } from 'child_process';

const GITHUB_REPO = 'hayalpkgulec-arch/codai-ollama-extension';
const GITHUB_API = `https://api.github.com/repos/${GITHUB_REPO}/releases/latest`;
const CHECK_INTERVAL = 30 * 60 * 1000;
const FOCUS_CHECK_MIN_INTERVAL = 5 * 60 * 1000;

type PendingInstall = {
    downloadUrl: string;
    version: string;
};

export class UpdaterService {
    private _timer?: ReturnType<typeof setInterval>;
    private _statusBar?: vscode.StatusBarItem;
    private _log: vscode.OutputChannel;
    private _pendingInstall?: PendingInstall;
    private _lastCheckAt = 0;
    private _lastNotifiedVersion?: string;
    private _isChecking = false;

    constructor(private readonly context: vscode.ExtensionContext) {
        this._log = vscode.window.createOutputChannel('CodAI Updater');
        context.subscriptions.push(this._log);
    }

    public start() {
        this._log.appendLine(`[Updater] start() current version: ${this.currentVersion()}`);
        this._log.appendLine(`[Updater] execPath: ${process.execPath}`);

        const checkCmd = vscode.commands.registerCommand('codai.checkUpdate', () => {
            this._log.appendLine('[Updater] Manual check triggered');
            this._log.show(true);
            void this.checkForUpdate(true, 'manual');
        });
        this.context.subscriptions.push(checkCmd);

        const installCmd = vscode.commands.registerCommand('codai.installUpdate', () => {
            if (!this._pendingInstall) {
                void vscode.window.showInformationMessage('CodAI icin bekleyen bir guncelleme yok.');
                return;
            }

            return this.installUpdate(this._pendingInstall.downloadUrl, this._pendingInstall.version);
        });
        this.context.subscriptions.push(installCmd);

        const focusCheck = vscode.window.onDidChangeWindowState((state) => {
            if (!state.focused) {
                return;
            }

            this._log.appendLine('[Updater] Window focused, checking for updates');
            void this.checkForUpdate(false, 'focus');
        });
        this.context.subscriptions.push(focusCheck);

        setTimeout(() => {
            this._log.appendLine('[Updater] Startup check...');
            void this.checkForUpdate(true, 'startup');
        }, 2000);

        this._timer = setInterval(() => {
            void this.checkForUpdate(false, 'interval');
        }, CHECK_INTERVAL);

        this.context.subscriptions.push({ dispose: () => this.dispose() });
    }

    public dispose() {
        if (this._timer) {
            clearInterval(this._timer);
        }
        this._statusBar?.dispose();
    }

    public async checkForUpdate(force = false, reason = 'scheduled'): Promise<void> {
        if (this._isChecking) {
            this._log.appendLine(`[Updater] Skipping ${reason} check: another check is already running`);
            return;
        }

        const now = Date.now();
        if (!force && reason === 'focus' && now - this._lastCheckAt < FOCUS_CHECK_MIN_INTERVAL) {
            this._log.appendLine(`[Updater] Skipping ${reason} check: throttled`);
            return;
        }

        this._isChecking = true;
        this._lastCheckAt = now;

        const current = this.currentVersion();
        this._log.appendLine(`[Updater] Checking (${reason})... current=${current}`);

        try {
            const release = await this.fetchLatestRelease();
            if (!release) {
                this._log.appendLine('[Updater] No release found');
                return;
            }

            const latest = String(release.tag_name || '').replace(/^v/, '');
            if (!latest) {
                this._log.appendLine('[Updater] Invalid release payload: missing tag_name');
                return;
            }

            const isNewer = this.isNewer(latest, current);
            this._log.appendLine(`[Updater] latest=${latest} current=${current} isNewer=${isNewer}`);

            if (!isNewer) {
                this._pendingInstall = undefined;
                this._lastNotifiedVersion = undefined;
                this._statusBar?.hide();
                this._log.appendLine('[Updater] Already up to date.');
                return;
            }

            const vsixAsset = Array.isArray(release.assets)
                ? release.assets.find((asset: any) => typeof asset?.name === 'string' && asset.name.endsWith('.vsix'))
                : undefined;

            if (!vsixAsset?.browser_download_url) {
                this._log.appendLine('[Updater] No .vsix asset found in release');
                return;
            }

            this._log.appendLine(`[Updater] Update available: ${current} -> ${latest}, asset=${vsixAsset.name}`);
            this.showUpdateNotification(current, latest, vsixAsset.browser_download_url);
        } catch (err: any) {
            this._log.appendLine(`[Updater] Check ERROR: ${err.message}`);
        } finally {
            this._isChecking = false;
        }
    }

    private showUpdateNotification(current: string, latest: string, downloadUrl: string) {
        this._pendingInstall = { downloadUrl, version: latest };

        if (!this._statusBar) {
            this._statusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 99);
            this.context.subscriptions.push(this._statusBar);
        }

        this._statusBar.command = 'codai.installUpdate';
        this._statusBar.text = `$(arrow-circle-up) CodAI ${latest}`;
        this._statusBar.tooltip = `CodAI guncellemesi: v${current} -> v${latest}. Tikla ve yukle.`;
        this._statusBar.backgroundColor = new vscode.ThemeColor('statusBarItem.warningBackground');
        this._statusBar.show();

        if (this._lastNotifiedVersion === latest) {
            this._log.appendLine(`[Updater] Update ${latest} already announced, keeping status bar visible`);
            return;
        }

        this._lastNotifiedVersion = latest;
        void vscode.window.showInformationMessage(
            `CodAI guncellemesi mevcut: v${current} -> v${latest}`,
            'Simdi Guncelle',
            'Sonra'
        ).then((choice) => {
            if (choice === 'Simdi Guncelle') {
                void this.installUpdate(downloadUrl, latest);
            }
        });
    }

    public async installUpdate(downloadUrl: string, version: string): Promise<void> {
        this._log.appendLine(`[Updater] Installing v${version} from ${downloadUrl}`);

        await vscode.window.withProgress(
            {
                location: vscode.ProgressLocation.Notification,
                title: `CodAI v${version} yukleniyor...`,
                cancellable: false,
            },
            async (progress) => {
                try {
                    progress.report({ increment: 10, message: 'Indiriliyor...' });
                    const vsixPath = await this.downloadFile(downloadUrl, `codai-${version}.vsix`);
                    this._log.appendLine(`[Updater] Downloaded to ${vsixPath}`);

                    progress.report({ increment: 60, message: 'Yukleniyor...' });
                    await this.installVsix(vsixPath);
                    this._log.appendLine('[Updater] Installed successfully');

                    progress.report({ increment: 30, message: 'Tamamlandi' });
                    try {
                        fs.unlinkSync(vsixPath);
                    } catch {
                        // ignore temporary file cleanup failures
                    }

                    if (this._pendingInstall?.version === version) {
                        this._pendingInstall = undefined;
                    }
                    this._statusBar?.hide();

                    const reload = await vscode.window.showInformationMessage(
                        `CodAI v${version} yuklendi. Degisikliklerin aktif olmasi icin yeniden yukle.`,
                        'Simdi Yeniden Yukle'
                    );
                    if (reload) {
                        await vscode.commands.executeCommand('workbench.action.reloadWindow');
                    }
                } catch (err: any) {
                    this._log.appendLine(`[Updater] Install ERROR: ${err.message}`);
                    void vscode.window.showErrorMessage(`CodAI guncelleme hatasi: ${err.message}`);
                }
            }
        );
    }

    private fetchLatestRelease(): Promise<any> {
        return new Promise((resolve, reject) => {
            const options = {
                headers: {
                    'User-Agent': 'CodAI-Updater/1.0',
                    Accept: 'application/vnd.github.v3+json',
                },
            };

            this._log.appendLine(`[Updater] GET ${GITHUB_API}`);
            https
                .get(GITHUB_API, options, (res) => {
                    this._log.appendLine(`[Updater] HTTP ${res.statusCode}`);

                    if (res.statusCode === 404) {
                        resolve(null);
                        return;
                    }

                    if (res.statusCode !== 200) {
                        reject(new Error(`GitHub API: ${res.statusCode}`));
                        return;
                    }

                    let data = '';
                    res.on('data', (chunk) => {
                        data += chunk;
                    });
                    res.on('end', () => {
                        try {
                            resolve(JSON.parse(data));
                        } catch (err) {
                            reject(err);
                        }
                    });
                })
                .on('error', (err) => {
                    this._log.appendLine(`[Updater] Network error: ${err.message}`);
                    reject(err);
                });
        });
    }

    private downloadFile(url: string, filename: string): Promise<string> {
        const dest = path.join(os.tmpdir(), filename);

        return new Promise((resolve, reject) => {
            const follow = (redirectUrl: string) => {
                this._log.appendLine(`[Updater] Downloading: ${redirectUrl}`);
                https
                    .get(redirectUrl, { headers: { 'User-Agent': 'CodAI-Updater/1.0' } }, (res) => {
                        if ((res.statusCode === 301 || res.statusCode === 302) && res.headers.location) {
                            follow(res.headers.location);
                            return;
                        }

                        if (res.statusCode !== 200) {
                            reject(new Error(`Download failed: ${res.statusCode}`));
                            return;
                        }

                        const file = fs.createWriteStream(dest);
                        res.pipe(file);
                        file.on('finish', () => file.close(() => resolve(dest)));
                        file.on('error', (err) => {
                            fs.unlink(dest, () => undefined);
                            reject(err);
                        });
                    })
                    .on('error', reject);
            };

            follow(url);
        });
    }

    private installVsix(vsixPath: string): Promise<void> {
        const candidates = this.buildCliCandidates();
        this._log.appendLine(`[Updater] CLI candidates: ${candidates.join(', ')}`);

        return new Promise((resolve, reject) => {
            this.tryExecCandidates(candidates, vsixPath, resolve, reject);
        });
    }

    private buildCliCandidates(): string[] {
        const execDir = path.dirname(process.execPath);

        if (process.platform === 'win32') {
            return [
                path.join(execDir, 'bin', 'codium.cmd'),
                path.join(execDir, 'bin', 'code.cmd'),
                path.join(execDir, 'resources', 'app', 'bin', 'code.cmd'),
                'codium.cmd',
                'code.cmd',
                'codium',
                'code',
            ];
        }

        return [
            path.join(execDir, 'bin', 'codium'),
            path.join(execDir, 'bin', 'code'),
            'codium',
            'code',
        ];
    }

    private tryExecCandidates(
        candidates: string[],
        vsixPath: string,
        resolve: () => void,
        reject: (error: Error) => void
    ) {
        if (candidates.length === 0) {
            reject(new Error('No suitable VS Code / VSCodium CLI found. Please install manually.'));
            return;
        }

        const [current, ...rest] = candidates;
        this._log.appendLine(`[Updater] Trying CLI: ${current}`);

        const isWinCmd = process.platform === 'win32' && current.toLowerCase().endsWith('.cmd');
        const bin = isWinCmd ? 'cmd.exe' : current;
        const args = isWinCmd
            ? ['/c', current, '--install-extension', vsixPath, '--force']
            : ['--install-extension', vsixPath, '--force'];

        execFile(bin, args, (err, stdout) => {
            if (err) {
                this._log.appendLine(`[Updater] Failed (${current}): ${err.message.split('\n')[0]}`);
                this.tryExecCandidates(rest, vsixPath, resolve, reject);
                return;
            }

            this._log.appendLine(`[Updater] Success with: ${current}\n${stdout}`);
            resolve();
        });
    }

    private currentVersion(): string {
        return this.context.extension.packageJSON.version as string;
    }

    private isNewer(latest: string, current: string): boolean {
        const parse = (value: string) => value.split('.').map((segment) => Number(segment) || 0);
        const [latestMajor, latestMinor, latestPatch] = parse(latest);
        const [currentMajor, currentMinor, currentPatch] = parse(current);

        if (latestMajor !== currentMajor) {
            return latestMajor > currentMajor;
        }

        if (latestMinor !== currentMinor) {
            return latestMinor > currentMinor;
        }

        return latestPatch > currentPatch;
    }
}
